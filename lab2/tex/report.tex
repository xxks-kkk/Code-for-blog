...
\texttt{tcpdump} command can dump traffic on a network interface.  We follow the instruction and use two packets to capture packets. On one terminal, we run 
\lstinline|sudo tcpdump -i eno1 -w tcpdump.out.1| and on the other terminal, we first release our machine's current DHCP lease by executing \lstinline|dhclient -r| and recontact DHCP server via \lstinline|sudo dhclient eno1|. We stop capturing once \lstinline|sudo dhclient eno1| finishes.  We answer the following questions based on the packets we record.
...
\begin{QandA}
	...
	\begin{lstlisting}
19:07:21.409099 IP 0.0.0.0.68 > 255.255.255.255.67: BOOTP/DHCP, Request from 10:98:36:ac:9d:45, length 300
	\end{lstlisting}
	...
	\begin{lstlisting}
19:07:41.375989 10:98:36:ac:9d:45 > ff:ff:ff:ff:ff:ff, ethertype IPv4 (0x0800), length 342: 0.0.0.0.68 > 255.255.255.255.67: BOOTP/DHCP, Request from 10:98:36:ac:9d:45, length 300
19:07:44.654365 d4:ae:52:e8:4e:85 > 10:98:36:ac:9d:47, ethertype IPv4 (0x0800), length 342: 192.168.1.1.67 > 192.168.1.221.68: BOOTP/DHCP, Reply, length 300
	\end{lstlisting}
	From the printout we can see that the link-layer address (MAC address) of the host is \texttt{10:98:36:ac:9d:45}.
	\end{answered}
	...
	\item What is the purpose of the DHCP release message?  What would happen if the client's DHCP release message is lost?
	\begin{answered}
		\lipsum[2]
	\end{answered}
