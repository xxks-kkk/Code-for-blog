...
class Counter(object):
    def __init__(self, ls=None):
        if ls is None:
            ...
            self.counter = collections.Counter(ls)
    ...
    def subtract(self, otherCounter):
        for key in otherCounter.counter.keys():
            self.increment_count(key, -otherCounter.counter[key])
    ...
    def add(self, elt, score, state):
        ...
        # Find the insertion point with binary search
            ...
            if len(self.scores) > self.size:
                del self.elts_to_states[self.elts[-1]]
                self.elts.pop()
                self.scores.pop()
