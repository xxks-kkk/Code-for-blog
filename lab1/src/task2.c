...
int hw_cache_perf_event_open(int group_fd, int cache_id, int cache_op_id, int cache_op_result_id) {
...
int main(int argc, char** argv) {
  if (argc < 8) {
    fprintf(stderr, "Needs 7 arguments: cpu_id opt_random_access mmap_type mmap_populate init_memset counter_group bg_compete\n");
    exit(EXIT_FAILURE);
  }
  ...
  int l1_read_access_fd = hw_cache_perf_event_open(-1, PERF_COUNT_HW_CACHE_L1D,
      PERF_COUNT_HW_CACHE_OP_READ, PERF_COUNT_HW_CACHE_RESULT_ACCESS);
  int leader_fd = l1_read_access_fd;

  int l1_read_miss_fd = hw_cache_perf_event_open(leader_fd, PERF_COUNT_HW_CACHE_L1D,
      PERF_COUNT_HW_CACHE_OP_READ, PERF_COUNT_HW_CACHE_RESULT_MISS);
  int l1_write_access_fd = hw_cache_perf_event_open(leader_fd, PERF_COUNT_HW_CACHE_L1D,
      PERF_COUNT_HW_CACHE_OP_WRITE, PERF_COUNT_HW_CACHE_RESULT_ACCESS);

  int tlb_read_miss_fd = hw_cache_perf_event_open(-1, PERF_COUNT_HW_CACHE_DTLB,
      PERF_COUNT_HW_CACHE_OP_READ, PERF_COUNT_HW_CACHE_RESULT_MISS);
  int leader2_fd = tlb_read_miss_fd;
  int tlb_write_miss_fd = hw_cache_perf_event_open(leader2_fd, PERF_COUNT_HW_CACHE_DTLB,
  ...
  printf("utime = %ld.%06ld s\n", before_usage.ru_utime.tv_sec, before_usage.ru_utime.tv_usec);
  printf("stime = %ld.%06ld s\n", before_usage.ru_stime.tv_sec, before_usage.ru_stime.tv_usec);
  ...
  printf("[Delta]\n");
  double d_utime = TIMEVAL_TO_DOUBLE(after_usage.ru_utime) - TIMEVAL_TO_DOUBLE(before_usage.ru_utime);
  double d_stime = TIMEVAL_TO_DOUBLE(after_usage.ru_stime) - TIMEVAL_TO_DOUBLE(before_usage.ru_stime);
  printf("utime = %.6f s\n", d_utime);
  printf("stime = %.6f s\n", d_stime);
