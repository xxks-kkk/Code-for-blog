...
level 1 data cache accesses and misses for trivial \texttt{printf}.
...
grouped together so that we can measure corresponding events simultaneously. There are five events we want to measure and ideally, we want to group them together and measure them at the same time. However, CPU only has limited amount of registers that can be used as performance counters. 
