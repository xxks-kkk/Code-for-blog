...
public class BackwardBigramModel {
    ...
    public void trainSentences(List<List<String>> sentences) {
        for (List<String> sentence : sentences) {
            // We reverse the direction of sentence because of the backward model
    ...
    public void trainSentence(List<String> sentence) {
        // Precondition: we assume sentence is already reversed
        // First count an initial start sentence token
        // In the forward model, we have <s>A,B,C</s>. Now, in the backward model,
        // we have </s>C,B,A<s>
        String prevToken = "</S>";
    ...
    public double sentenceLogProb(List<String> sentence) {
        // Precondition: we assume sentence is already reversed
        // Set start-sentence as initial token. Again, since we are in the backward
        // model, we start with </S>.
        String prevToken = "</S>";
    ...
    public double sentenceLogProb2(List<String> sentence) {
        // precondition: we assume sentence is already reversed
        // We're in the backward model, so start with </S>
        String prevToken = "</S>";
        double sentenceLogProb = 0;
        for (String token : sentence) {
            DoubleValue unigramVal = unigramMap.get(token);
    ...
    public double[] sentenceTokenProbs(List<String> sentence) {
        // Set end-sentence </S> as initial token
        String prevToken = "</S>";
