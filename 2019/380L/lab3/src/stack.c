...
void *build_stack(void *rsp, struct elf_file_t *elf_file, int argc,
  ...
  uint64_t phdr = elf_file->program_header_table[0].p_vaddr +
                  elf_file->file_header->e_phoff;
  LOG("phdr = 0x%lx", (long)phdr);
  rsp = push_auxv_with_val(rsp, AT_PHDR, phdr);
  rsp = push_auxv_with_val(rsp, AT_PHENT, elf_file->file_header->e_phentsize);
  rsp = push_auxv_with_val(rsp, AT_PHNUM, elf_file->file_header->e_phnum);
