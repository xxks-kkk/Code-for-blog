...
int load_page(long addr) {
  ...
  for (int i = 0; i < elf_file->file_header->e_phnum; i++) {
    ...
    if (segment->p_type == PT_LOAD) {
      long vaddr = (long)segment->p_vaddr;
      long length = (long)segment->p_memsz;
      ...
      if (mmap((void *)page_start_addr, page_size, PROT_WRITE, flags, -1, 0) ==
          MAP_FAILED) {
        LOG("mmap failed: %s", strerror(errno));
        return -1;
      }

      long filesz = (long)segment->p_filesz;
      long offset = (long)segment->p_offset;
...
int main(int argc, char **argv) {
  ...
  CHECK(prefetch_num == 0 || prefetch_num == 1 || prefetch_num == 2,
        "Invalid prefetch_num");
  ...
  for (int i = 0; i < elf_file->file_header->e_phnum; i++) {
    ...
    if (segment->p_type == PT_LOAD) {
      long vaddr = (long)segment->p_vaddr;
      long align = (long)segment->p_align;
      ...
      LOG("vaddr = 0x%lx, align = 0x%lx, aligned_vaddr = 0x%lx", vaddr, align,
          aligned_vaddr);
      ...
      offset -= align_delta;
      LOG("length = 0x%lx, filesz = 0x%lx, offset = 0x%lx", length, filesz,
          offset);
