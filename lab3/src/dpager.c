...
int load_page(size_t addr) {
    for (int i = 0; i < elf_file->n_program_headers; i++) {
        ...
        if (segment->type == PT_LOAD) {
            ...
            if (mmap(page_start_addr, page_size, PROT_WRITE, flags, -1, 0) != page_start_addr) {
                LOG("mmap failed");
                return -1;
            }

            int64_t filesz = (int64_t) segment->filesz;
            int64_t offset = (int64_t) segment->offset;
            ...
            if (offset_in_segment < filesz) {
                ...
                memcpy(page_start_addr, elf_file->content + offset + offset_in_segment, copy_size);
            }
