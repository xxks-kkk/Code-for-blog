...
int main(int argc, char **argv) {
  ...
  printf("p_type = 0x%x\n", elf_file->program_header->p_type);
  printf("p_offset = 0x%lx\n", elf_file->program_header->p_offset);
  printf("p_vaddr = 0x%lx\n", elf_file->program_header->p_vaddr);
  printf("p_paddr = 0x%lx\n", elf_file->program_header->p_paddr);
  printf("p_filesz = 0x%lx\n", elf_file->program_header->p_filesz);
  printf("p_memsz = 0x%lx\n", elf_file->program_header->p_memsz);
  printf("p_flags = 0x%x\n", elf_file->program_header->p_flags);
  printf("p_align = 0x%lx\n", elf_file->program_header->p_align);
  printf("\n");

  printf("[Section header]\n");
  printf("sh_name = 0x%x\n", elf_file->section_header->sh_name);
  printf("sh_type = 0x%x\n", elf_file->section_header->sh_type);
  printf("sh_flags = 0x%lx\n", elf_file->section_header->sh_flags);
  printf("sh_addr = 0x%lx\n", elf_file->section_header->sh_addr);
  printf("sh_offset = 0x%lx\n", elf_file->section_header->sh_offset);
  printf("sh_size = 0x%lx\n", elf_file->section_header->sh_size);
  printf("sh_link = 0x%x\n", elf_file->section_header->sh_link);
  printf("sh_info = 0x%x\n", elf_file->section_header->sh_info);
  printf("sh_addralign = 0x%lx\n", elf_file->section_header->sh_addralign);
  printf("sh_entsize = 0x%lx\n", elf_file->section_header->sh_entsize);
  printf("\n");
